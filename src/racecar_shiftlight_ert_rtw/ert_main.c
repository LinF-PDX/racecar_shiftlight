...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    racecar_shiftlight_M->Timing.clockTick0;
...
int main(void)
{
  ...
  errorCode = extmodeInit(racecar_shiftlight_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(racecar_shiftlight_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(racecar_shiftlight_M);
  enableTimer0Interrupt();
  config_ePWM_TBSync();
  StartDMA();
  globalInterruptEnable();
